...
export const BlacklistManager: React.FC<BlacklistManagerProps> = ({ className = '' }) => {
  ...
  const [newProgramReason, setNewProgramReason] = useState('');
  ...
  const handleAddProgram = async () => {
    ...
    const reason = newProgramReason.trim();
    ...
    try {
      await addToBlacklist(programId, reason);
      setNewProgramId('');
      setNewProgramReason('');
      setError(null);
    } catch (err: unknown) {
  ...
  const handleClearAll = async () => {
    if (confirm('Are you sure you want to clear all blacklisted programs? This will affect all users.')) {
      try {
        ...
        <div className="absolute top-full mt-2 left-0 w-96 bg-gray-900 border border-gray-700 rounded-md shadow-lg z-50">
          <div className="p-4 border-b border-gray-700">
            ...
            <p className="text-sm text-gray-400 mb-3">
              Hide specific programs from all search results and comparisons (server-side, affects all users)
            </p>
            ...
            <div className="space-y-2">
              ...
              <input
                type="text"
                value={newProgramReason}
                onChange={(e) => setNewProgramReason(e.target.value)}
                onKeyPress={handleKeyPress}
